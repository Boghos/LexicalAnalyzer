...
public class lexicalAnalizer {
    ...
    public static void analyze(String text) {
        ...
        if (!checkParenthesesBalance(tokens)) {
            System.out.println("Error: Mismatched parentheses detected.");
            isValid = false;
        }
        ...
        // Check for incorrect operator usage
        if (!checkOperatorUsage(tokens)) {
            System.out.println("Error: Incorrect operator usage detected.");
            isValid = false;
        }
    ...
    private static String readFile(String filePath) {
        try {
            return new String(Files.readAllBytes(Paths.get(filePath)));  // Read all lines from the file
        } catch (IOException e) {
    ...
    // Check for mismatched parentheses using a stack
    private static boolean checkParenthesesBalance(String[] tokens) {
        ...
        return stack.isEmpty();  // Returns true if all parentheses are matched
    }

    // Check for incorrect operator usage
    private static boolean checkOperatorUsage(String[] tokens) {
        for (int i = 0; i < tokens.length; i++) {
            if (isOperator(tokens[i])) {
                // Ensure operators are not at the start or end and are surrounded by valid tokens
                if (i == 0 || i == tokens.length - 1 || isOperator(tokens[i - 1]) || isOperator(tokens[i + 1]) ||
                        isSeparator(tokens[i - 1]) || isSeparator(tokens[i + 1])) {
